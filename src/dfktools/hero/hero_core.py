...
def transfer_hero_and_equipment_from(contract_address, hero_id, receiver_address, private_key, nonce, gas_price_gwei, tx_timeout_seconds, rpc_address, logger=None):
    ...
    tx = contract.functions.transferHeroAndEquipmentFrom(owner, receiver_address, hero_id)
...
def get_heroes_v3(contract_address, hero_ids, rpc_address, block_identifier="latest"):
    w3 = Web3(Web3.HTTPProvider(rpc_address))

    contract_address = Web3.to_checksum_address(contract_address)
    contract = w3.eth.contract(contract_address, abi=ABI)
    hero_structs = contract.functions.getHeroesV3(hero_ids).call(block_identifier=block_identifier)
    result = []
    for x in hero_structs:
        result.append(_parse_hero_struct(x))
